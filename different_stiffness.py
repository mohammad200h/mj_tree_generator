...
if __name__ =="__main__":
  ...
  for s in tqdm(range(stiffness["start"],stiffness["end"],stiffness["step"])):
    print(f"-----recording for stiffness: {s}----")
    arena = Arena()
    root, spec , bodies = tree(stiffness = s, damping=10)
    # removing contacts
    for body in bodies:
...
#     print(f"warning::{data.warning}")
    ...
    # Define video writer
    out = cv2.VideoWriter('stiffness_'+str(s)+'.mp4', cv2.VideoWriter_fourcc(*'mp4v'), framerate, (r_width, r_height))
    for frame in frames:
        out.write(cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))  # Convert RGB to BGR for OpenCV

    out.release()
    print("Video saved as "+'stiffness_'+str(s)+'.mp4')
